...
//    int threadId1; // ipv rayidx
    ...
    // Initialize (stores local variables in registers)
    {
        ...
        // vec4 o = rays[rayidx * 2 + 0];
        // vec4 d = rays[rayidx * 2 + 1];
    ...
    while (nodeAddr != EntrypointSentinel)
    {
        ...
        while (nodeAddr >= 0 && nodeAddr != EntrypointSentinel)
        {
            ...
            vec4 nz = ptr[2]; // childnode 0 and 1, z-bounds (c0.lo.z, c0.hi.z, c1.lo.z, c1.hi.z)
                ...
                if (t > tmin && t < hitT)
                {
                    ...
                    if (u >= 0.0f && u <= 1.0f)
                    {
                        ...
                        if (v >= 0.0f && u + v <= 1.0f)
                        {
                            ...
                            //trinormal = cross(vec3(v22.x, v22.y, v22.z), vec3(v11.x, v11.y, v11.z));  // works
...
__device__ void DEBUGintersectBVHandTriangles(const glm::vec4 rayorig, const vec4 raydir,
                                              const glm::vec4 * gpuNodes, const glm::vec4 *  gpuDebugTris, const int* gpuTriIndices,
